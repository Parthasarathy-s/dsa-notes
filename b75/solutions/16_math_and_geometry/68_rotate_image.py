"""
Rotate Image
--------------
You are given an n x n 2D `matrix` representing an image. Rotate the
image by 90 degrees (clockwise), in place.

You have to rotate the image in place, meaning you have to modify the
input 2D matrix directly. Do not allocate another 2D matrix and do the
rotation.

Example 1:
    Input: matrix = [[1,2,3],[4,5,6],[7,8,9]]
    Output: [[7,4,1],[8,5,2],[9,6,3]]

Example 2:
    Input: matrix = [[5,1,9,11],[2,4,8,10],[13,3,6,7],[15,14,12,16]]
    Output: [[15,13,2,5],[14,3,4,1],[12,6,8,9],[16,7,10,11]]

Constraints:
    n == len(matrix) == len(matrix[i])
    1 <= n <= 20
    -1000 <= matrix[i][j] <= 1000
"""

from typing import List


def rotate(matrix: List[List[int]]) -> None:
    """Rotate the matrix in place. Do not return anything."""
    n = len(matrix)

    # Transpose the matrix in place.
    for i in range(n):
        for j in range(i + 1, n):
            matrix[i][j], matrix[j][i] = matrix[j][i], matrix[i][j]

    # Reverse each row to get the clockwise rotation.
    for row in matrix:
        row.reverse()


if __name__ == "__main__":
    tests = [
        ([[1, 2, 3], [4, 5, 6], [7, 8, 9]], [[7, 4, 1], [8, 5, 2], [9, 6, 3]]),
        (
            [[5, 1, 9, 11], [2, 4, 8, 10], [13, 3, 6, 7], [15, 14, 12, 16]],
            [[15, 13, 2, 5], [14, 3, 4, 1], [12, 6, 8, 9], [16, 7, 10, 11]],
        ),
    ]

    for matrix, expected in tests:
        rotate(matrix)
        status = "PASS" if matrix == expected else "FAIL"
        print(f"{status}: rotate(...) -> {matrix} (expected {expected})")
